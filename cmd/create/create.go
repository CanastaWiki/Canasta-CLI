...
import (
	...
	"github.com/CanastaWiki/Canasta-CLI/internal/prompt"
)
...
func NewCmdCreate() *cobra.Command {
	...
	createCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err = createCanasta(canastaInfo, pwd, path, name, domain, yamlPath, orchestrator, override, test); err != nil {
				fmt.Print(err.Error(), "\n")
				if keepConfig {
	...
	createCmd.Flags().BoolVarP(&test, "test", "t", false, "Use test docker compose file or not")

	return createCmd
}

// importCanasta accepts all the keyword arguments and create a installation of the latest Canasta.
func createCanasta(canastaInfo canasta.CanastaVariables, pwd, path, name, domain, yamlPath, orchestrator, override string, test bool) error {
	...
	if err := canasta.CloneStackRepo(orchestrator, canastaInfo.Id, &path, test); err != nil {
		return err
	}
