...
Default file contents (READMEs, config templates) are stored in `internal/canasta/files/` and loaded at compile time via `go:embed`. When adding a new template file:

1. Create the file in `internal/canasta/files/`
2. Add a `//go:embed files/<filename>` directive and variable in `internal/canasta/canasta.go`
3. Reference the variable where the file is written
...
- The tool must typically run with sudo/root for docker access and config file permissions
...
- The CLI checks for docker compose availability on init via `orchestrators.CheckDependencies()`
