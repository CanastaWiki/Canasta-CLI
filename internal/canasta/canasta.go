...
func RewriteCaddy(installPath string) error {
	...
	// Generate the site address
	var siteAddress strings.Builder
	if sslTermination {
		// SSL termination: listen on port 80 only
		siteAddress.WriteString(":80")
	} else {
		// Normal HTTPS: use FQDNs (Caddy auto-provisions certificates)
	...
	fmt.Fprintln(writer, "# Auto-generated by Canasta CLI — DO NOT EDIT")
	fmt.Fprintln(writer, "# To add custom Caddy directives, edit Caddyfile.custom instead.")
	fmt.Fprintln(writer, "# For global Caddy options, edit Caddyfile.global instead.")
	fmt.Fprintln(writer, "")

	// Write global options block
	fmt.Fprintln(writer, "{")
	if sslTermination {
		fmt.Fprintln(writer, "    auto_https off")
	}
	fmt.Fprintln(writer, "    import /etc/caddy/Caddyfile.global")
	fmt.Fprintln(writer, "}")
	fmt.Fprintln(writer, "")

	// Write site block with curly braces
	fmt.Fprintln(writer, siteAddress.String()+" {")
	fmt.Fprintln(writer, "    import /etc/caddy/Caddyfile.custom")
	if sslTermination {
		// SSL termination: preserve Host header through proxy chain
		fmt.Fprintln(writer, "    reverse_proxy varnish:80 {")
		fmt.Fprintln(writer, "        header_up Host {host}")
		fmt.Fprintln(writer, "    }")
	} else {
		fmt.Fprintln(writer, "    reverse_proxy varnish:80")
	}
...
// CreateCaddyfileCustom creates config/Caddyfile.custom with a template comment header.
// It only writes the file if it doesn't already exist (no-clobber).
func CreateCaddyfileCustom(installPath string) error {
	filePath := filepath.Join(installPath, "config", "Caddyfile.custom")
