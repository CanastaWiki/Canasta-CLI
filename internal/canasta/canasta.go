...
func RewriteCaddy(installPath string) error {
	...
	writeLine("# To add custom Caddy directives, edit Caddyfile.custom instead.")
	writeLine("# For global Caddy options, edit Caddyfile.global instead.")
	writeLine("")

	// Write global options block
	writeLine("{")
	if sslTermination {
		writeLine("    auto_https off")
	}
