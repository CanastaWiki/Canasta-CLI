...
# sidecar receives — "define once in .env, deliver to both".
_BRIDGE_HEADER = (
    "<?php\n"
...
def collect_env_refs(sidecars):
    """Instance .env var names referenced by any sidecar (env values + command)."""
    names = set()
    for sidecar in sidecars or []:
        values = list((sidecar.get("env") or {}).values())
