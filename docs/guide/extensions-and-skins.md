...
When you run `canasta extension enable VisualEditor`, the CLI adds the extension name to a YAML file called `settings.yaml` on the host filesystem:

- **Global enable** updates `config/settings/global/settings.yaml`:
  ```yaml
  extensions:
  - VisualEditor
  ```

- **Per-wiki enable** (`-w docs`) updates `config/settings/wikis/docs/settings.yaml` with the same format.
...
- **Disable only works on CLI-managed extensions.** Only entries in `settings.yaml` are affected; other files are left alone.
